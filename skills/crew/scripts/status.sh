#!/bin/sh
# Exit statuses:
#   0  A read-only snapshot was printed; this never means the run is complete.
...
def field(lines: list[str], name: str, pattern: str) -> str | None:
    ...
    match = re.fullmatch(rf"- {name}:\s*({pattern})(?:\s+[-—].*)?\s*", candidates[0]) if len(candidates) == 1 else None
...
def read_run(state_root: Path, cwd: Path) -> tuple[Path | None, list[str]]:
    ...
    if availability == "absent":
        return None, []
    if availability != "present" or not re.fullmatch(r"[A-Za-z0-9][A-Za-z0-9._-]{0,127}\n?", pointer or ""):
        run["availability"] = "unavailable"
        attention("run-pointer-unavailable", "The active-run pointer is unreadable or invalid.", "Inspect the external .current record; do not replace it automatically.")
        return None, []
    run["id"] = pointer.rstrip("\n")
    run_dir = cwd / ".crew" / run["id"]
    if not directory(cwd / ".crew") or not directory(run_dir):
        attention("run-directory-unavailable", "The active run directory is missing or unsafe to inspect.", "Check the working directory and the recorded run directory.")
        return None, []
    availability, text = read_file(run_dir / "state.md")
    if availability != "present":
        attention("run-state-unavailable", "The active run has no readable state.md.", "Inspect the run artifacts before deciding its phase or round.")
        return run_dir, []
    lines = prose(text)
...
def read_partner(state_root: Path, state_lines: list[str]) -> None:
    ...
    if availability == "absent":
        if snapshot["run"]["phase"] not in (None, 0, "Finishing"):
            attention("partner-record-absent", "The recorded phase has no partner receipt.", "Check the run notes and external worker.json before contacting a worker.")
        return
    try:
        ...
        valid = (isinstance(receipt, dict) and receipt.get("version") == 1
                 and all(isinstance(receipt.get(key), str) and re.fullmatch(r"[A-Za-z0-9_-]+", receipt[key]) for key in identity)
                 and re.fullmatch(r"[a-z][a-z0-9_-]{0,31}", receipt["worker_name"]))
    except (ValueError, TypeError):
        valid = False
    if not valid:
        partner["record"] = "unavailable"
        attention("partner-record-unavailable", "The partner receipt is unreadable or invalid.", "Inspect external worker.json; do not infer an identity from pane position.")
        return
    ...
    for label, key in (("Worker", "name"), ("Pane", "pane_id")):
        ...
        if recorded and recorded != "none" and recorded != partner[key]:
            attention("run-partner-mismatch", f"The recorded {label.lower()} disagrees with the partner receipt.", "Compare state.md with worker.json before resuming work.")
    partner["observation"] = "unavailable"
    try:
        result = subprocess.run(["herdr", "agent", "get", partner["name"]], capture_output=True, text=True, timeout=5)
        if result.returncode:
            for output in (result.stdout, result.stderr):
                try:
                    ...
                    continue
                if isinstance(payload, dict) and isinstance(payload.get("error"), dict) and payload["error"].get("code") == "agent_not_found":
                    partner["observation"] = "absent"
                    attention("partner-absent", "The recorded partner is not present in Herdr.", "Check the recorded pane and run artifacts; absence does not authorize replacement or retirement.")
                    return
            raise ValueError("unavailable")
        agent = json.loads(result.stdout)["result"]["agent"]
        if not isinstance(agent, dict):
            raise ValueError("invalid response")
        if any(agent.get(source) != partner[target] for source, target in (("name", "name"), ("agent", "kind"), ("pane_id", "pane_id"))):
            partner["observation"] = "mismatch"
            attention("live-partner-mismatch", "The live identity does not match the recorded partner.", "Verify the recorded name, kind, and pane before taking any action.")
            return
        ...
        if status in ("blocked", "unknown"):
            attention("worker-" + status, "The worker reports " + status + ".", "Inspect the worker directly and follow the supervision rules; status inspection sends no input.")
    except (OSError, UnicodeError, ValueError, KeyError, TypeError, subprocess.TimeoutExpired):
        attention("herdr-unavailable", "The partner state could not be verified through Herdr.", "Check Herdr availability and repeat the read-only inspection.")
...
def read_artifacts(run_dir: Path) -> None:
    ...
    for round_number in range(1, 4):
        for kind in ("report", "review"):
            ...
            if availability != "present":
                attention("artifact-unavailable", f"{filename} is unreadable or unsafe to inspect.", "Inspect the named artifact; its contents are not included in this summary.")
                continue
            lines = prose(text)
            if kind == "report":
                try:
                    result = subprocess.run([str(helpers / "artifact-done.sh"), str(path)], capture_output=True, timeout=5)
                    if result.returncode not in (0, 1):
                        raise ValueError("artifact check failed")
                    entry["complete"] = result.returncode == 0
                    entry["self_review_complete"] = entry["complete"] and "## Phase 3 self-review" in lines
                except (OSError, ValueError, subprocess.TimeoutExpired):
                    attention("artifact-check-unavailable", f"The completion check for {filename} failed.", "Check artifact-done.sh and the report; do not infer completion from worker state.")
                reports[round_number] = entry
                if entry["complete"]:
                    artifacts["latest_completed_report"] = entry
            else:
                # Only explicit verdict lines are interpreted; findings or quoted examples are not verdicts.
                verdicts = []
                for line in lines:
                    explicit = line.replace("**", "").replace("`", "")
                    match = re.fullmatch(r"(?:- )?(?:Verdict:\s*)?(approve-with-nits|approve|block)", explicit)
                    if match:
    ...
    if artifacts["latest_review"] and artifacts["latest_review"]["verdict"] == "block":
        attention("review-block", "The latest review records block.", "Read the review and current round before deciding how to proceed.")


def inspect() -> None:
    try:
        result = subprocess.run([str(helpers / "state-root.sh")], capture_output=True, text=True, timeout=5)
        if result.returncode:
            raise ValueError("invalid root")
        state = json.loads(result.stdout)
        cwd, state_root = Path(state["cwd"]), Path(state["state_root"])
    except (OSError, UnicodeError, ValueError, KeyError, TypeError, subprocess.TimeoutExpired):
        attention("state-root-unavailable", "The workspace or external state root failed validation.", "Check the working directory and CREW_STATE_DIR using state-root.sh.")
        return
...
if sys.argv[2] == "--json":
    ...
    print(f"Partner record: {partner['record']}; {shown(partner['name'])} ({shown(partner['kind'])}), pane {shown(partner['pane_id'])}")
    print("Recorded lead pane:", shown(partner["lead_pane_id"]))
    print(f"Live observation: {partner['observation']}; agent status {shown(partner['agent_status'])}")
    for label, key in (("Latest report", "latest_report"), ("Latest completed report", "latest_completed_report"), ("Latest review", "latest_review")):
        item = artifacts[key]
        if item is None:
            ...
            print(f"{label}: {item['path']}; complete {shown(item['complete'])}; self-review complete {shown(item['self_review_complete'])}")
